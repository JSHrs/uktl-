...
export function validateAcceptanceProviders(env) {
  const required = ["ANTHROPIC_API_KEY", "RESEND_API_KEY", "REED_API_KEY", "CRON_SECRET", "CV_SCAN_URL", "CV_SCAN_TOKEN"];
...
export function buildStagingDeployment(env) {
  ...
  const config = {
    ...
    vars: {DATA_BACKEND:"supabase",SUPABASE_URL:env.SUPABASE_URL,SUPABASE_ANON_KEY:env.SUPABASE_ANON_KEY,SITE_URL:checked.origin,CV_SCAN_URL:env.CV_SCAN_URL,AI_HOURLY_CALL_LIMIT:env.AI_HOURLY_CALL_LIMIT||"100"},
    observability: {enabled:true},
  };
  const secrets = {};
  for (const key of ["DATABASE_URL","SUPABASE_SERVICE_ROLE_KEY","ANTHROPIC_API_KEY","RESEND_API_KEY","REED_API_KEY","CRON_SECRET","CV_SCAN_TOKEN"]) secrets[key] = env[key];
