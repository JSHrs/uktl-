# Current stage checkpoint — 20 September 2026

## Stage 2/3 acceptance recheck
...
- **Final verified source:** `1603ab58f0cce0a3fdf69cee35f549af51c09235` on `main`, [CI 35397232760](https://github.com/devacnt/UKTL/actions/runs/35397232760) SUCCESS. 58 unit tests, 28 browser checks, TypeScript/build and both complete CI jobs passed. Fresh PostgreSQL replay and actual HR ownership/source/booking replay/order/reschedule/email-mismatch tests passed with synthetic fixtures rolled back. Fourteen authenticated/live-credential browser tests remain skipped. Local HTTP smoke checks confirmed sign-in redirects for private HR pages and fail-closed webhook behavior; the local browser-only attempt was blocked by missing Chromium, then passed in CI's installed browser.
...
- Follow-up hardening includes bounded streaming AI responses, approval requiring existing video/caption object metadata, account appointment retrieval, extra reschedule/email-mismatch/privacy regressions and the compatible Seroval update from 1.5.2 to 1.6.7 for [GHSA-mv8w-475r-vwqw](https://github.com/advisories/GHSA-mv8w-475r-vwqw). Only that dependency lock entry was changed; unrelated lockfile churn was excluded. Final follow-up CI passed as recorded above.
- Dependency audit after that update: `npm audit --omit=dev` reports 0 critical, 11 high, 6 moderate and 2 low findings across runtime/build-tool dependency paths. Seroval is no longer flagged. These remaining advisories still require upgrade/impact review before release; a clean **database** security advisor does not mean the application dependency audit is clean.
